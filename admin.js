...
document.addEventListener('DOMContentLoaded', () => {
    ...
    (() => {
        ...
        onSnapshot(categoriesCollection, (snapshot) => { const cats = []; snapshot.forEach(doc => cats.push({ id: doc.id, ...doc.data() })); cats.sort((a, b) => (a.nombre || '').localeCompare(b.nombre || '')); const s = { docs: cats.map(c => ({ id: c.id, data: () => c })), forEach: (cb) => cats.forEach((c, i) => cb({ id: c.id, data: () => c }, i)), empty: cats.length === 0 }; render(s); updateDropdown(s); }, (e) => { console.error("Error categories: ", e); if(categoryList) categoryList.innerHTML = '<li class="list-group-item text-danger">Error.</li>'; });
    ...
    (() => {
        ...
        onSnapshot(suppliersCollection, (snapshot) => { const sups = []; snapshot.forEach(doc => sups.push({ id: doc.id, ...doc.data() })); sups.sort((a, b) => (a.nombre || '').localeCompare(b.nombre || '')); const s = { docs: sups.map(x => ({ id: x.id, data: () => x })), forEach: (cb) => sups.forEach((x, i) => cb({ id: x.id, data: () => x }, i)), empty: sups.length === 0 }; renderSuppliers(s); }, (e) => { console.error("Error suppliers:", e); if(listTable) listTable.innerHTML = '<tr><td colspan="4" class="text-center text-danger">Error.</td></tr>';});
    ...
    (() => {
        ...
        onSnapshot(clientsCollection, (snapshot) => { const cls = []; snapshot.forEach(doc => cls.push({ id: doc.id, ...doc.data() })); cls.sort((a, b) => (a.nombre || '').localeCompare(b.nombre || '')); const s = { docs: cls.map(x => ({ id: x.id, data: () => x })), forEach: (cb) => cls.forEach((x, i) => cb({ id: x.id, data: () => x }, i)), empty: cls.length === 0 }; renderClients(s); }, (e) => { console.error("Error clients:", e); if(clientListTable) clientListTable.innerHTML = '<tr><td colspan="6" class="text-center text-danger">Error.</td></tr>';});
    ...
    (() => {
         ...
         onSnapshot(repartidoresCollection, (snapshot) => { const reps = []; snapshot.forEach(doc => reps.push({ id: doc.id, ...doc.data() })); reps.sort((a, b) => (a.nombre || '').localeCompare(b.nombre || '')); const s = { docs: reps.map(x => ({ id: x.id, data: () => x })), forEach: (cb) => reps.forEach((x, i) => cb({ id: x.id, data: () => x }, i)), empty: reps.length === 0 }; renderRepartidores(s); }, (e) => { console.error("Error repartidores:", e); if(repartidorListTableBody) repartidorListTableBody.innerHTML = '<tr><td colspan="9" class="text-center text-danger">Error.</td></tr>';});
    ...
    (() => {
        ...
        onSnapshot(productsCollection, (snapshot) => { const prods = []; snapshot.forEach(doc => prods.push({ id: doc.id, ...doc.data() })); prods.sort((a, b) => { const tA = a.timestamp?.toMillis?.() || 0; const tB = b.timestamp?.toMillis?.() || 0; return tB - tA; }); const s = { docs: prods.map(p => ({ id: p.id, data: () => p })), forEach: (cb) => prods.forEach((p, i) => cb({ id: p.id, data: () => p }, i)), empty: prods.length === 0 }; renderProducts(s); }, e => { console.error("Error products:", e); if(productListTableBody) productListTableBody.innerHTML = '<tr><td colspan="8" class="text-center text-danger">Error.</td></tr>';});
        ...
        function loadCategoriesForFilter() {
            onSnapshot(categoriesCollection, (snapshot) => {
                categoriesMap.clear();
                const cats = [];
                snapshot.forEach(doc => cats.push({ id: doc.id, nombre: doc.data().nombre }));
                cats.sort((a, b) => (a.nombre || '').localeCompare(b.nombre || ''));
                cats.forEach(c => categoriesMap.set(c.id, c.nombre));
            });
        }
    ...
    (() => {
        ...
        if (closingForm) closingForm.addEventListener('submit', async (e) => { e.preventDefault(); showToast("Realizando cierre... (Lógica pendiente)", 'warning'); const egresos = parseFloat(document.getElementById('caja-egresos').value) || 0; const obs = document.getElementById('caja-observaciones').value.trim(); const cierreData = { timestamp: serverTimestamp(), egresos, observaciones: obs }; try { showToast("Cierre guardado (simulado)!"); } catch(err) { console.error("Err closing:", err); showToast(`Error: ${err.message}`, 'error'); } });
         ...
         onSnapshot(closingsCollection, (snapshot) => { const cls = []; snapshot.forEach(doc => cls.push({ id: doc.id, ...doc.data() })); cls.sort((a, b) => { const tA = a.timestamp?.toMillis?.() || 0; const tB = b.timestamp?.toMillis?.() || 0; return tB - tA; }); const s = { docs: cls.map(x => ({ id: x.id, data: () => x })), forEach: (cb) => cls.forEach((x, i) => cb({ id: x.id, data: () => x }, i)), empty: cls.length === 0 }; renderClosings(s); }, e => { console.error("Error closings:", e); if(closingHistoryTableBody) closingHistoryTableBody.innerHTML = '<tr><td colspan="7" class="text-center text-danger">Error.</td></tr>';});
    })();
